...
export const UI_COMPONENT_GUIDANCE: Readonly<Record<string, string>> = {
	migration_preview:
		...
		'props: { sourceLabel, targetLabel, mappings: [{ source, target, transform?, note? }], columns: [target column order], sampleRows: [[cells matching columns]], validations?: [{ row (0-BASED index into sampleRows: first row = 0), column, level: error|warning, message }], totalRowCount?, note? }. ' +
