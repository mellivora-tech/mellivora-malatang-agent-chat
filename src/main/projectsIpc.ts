...
export function registerProjectsIpc(dataRoot: string): void {
	registerHandler('projects:list', () => listProjects(dataRoot));
	registerHandler('projects:create', (_event, input: IProjectInput) => createProject(dataRoot, input));
	registerHandler('projects:delete', (_event, projectId: string) => deleteProject(dataRoot, projectId));
	...
	registerHandler('projects:listFiles', async (_event, projectId: string): Promise<readonly string[]> => {
	...
	registerHandler('projects:revealInFolder', async (_event, projectId: string): Promise<boolean> => {
		const project = await getProject(dataRoot, projectId);
		if (!project) {
			return false;
		}
	...
	registerHandler('projects:pickAndCreate', async (): Promise<IProject | undefined> => {
	...
	registerHandler('projects:listCodeRoots', async (_event, projectId: string): Promise<readonly ICodeRootView[]> => {
	...
	registerHandler('projects:discoverRepos', async (_event, _projectId: string, scanRoot?: string): Promise<readonly IDiscoveredRepo[]> => {
		return scanRepos(scanRoot && scanRoot.length > 0 ? scanRoot : homedir());
	});

	registerHandler('projects:addCodeRoot', async (_event, projectId: string, path: string): Promise<readonly ICodeRootView[]> => {
		await addCodeRoot(dataRoot, projectId, path);
		return listCodeRoots(dataRoot, projectId);
	});

	registerHandler('projects:removeCodeRoot', async (_event, projectId: string, path: string): Promise<readonly ICodeRootView[]> => {
		await removeCodeRoot(dataRoot, projectId, path);
		return listCodeRoots(dataRoot, projectId);
	});

	registerHandler('projects:listRemotes', (_event, projectId: string): Promise<readonly IRemoteRepoView[]> => listRemotes(dataRoot, projectId));
	...
	registerHandler('projects:listApprovalAllowlist', (_event, projectId: string): Promise<readonly string[]> => listProjectAllowPatterns(dataRoot, projectId));
	registerHandler('projects:removeApprovalAllowPattern', (_event, projectId: string, pattern: string): Promise<readonly string[]> =>
		removeProjectAllowPattern(dataRoot, projectId, pattern),
	);

	registerHandler('projects:addRemote', async (_event, projectId: string, input: IRemoteRepoInput): Promise<readonly IRemoteRepoView[]> => {
		await addRemote(dataRoot, projectId, input);
		return listRemotes(dataRoot, projectId);
	});

	registerHandler('projects:removeRemote', async (_event, projectId: string, remoteId: string): Promise<readonly IRemoteRepoView[]> => {
		await removeRemote(dataRoot, projectId, remoteId);
		return listRemotes(dataRoot, projectId);
	});

	registerHandler('projects:cloneRemote', (_event, projectId: string, remoteId: string): Promise<readonly IRemoteRepoView[]> =>
		cloneProjectRemote(dataRoot, projectId, remoteId, new AbortController().signal),
	);

	registerHandler('projects:pickCodeRoot', async (_event, projectId: string): Promise<readonly ICodeRootView[] | undefined> => {
