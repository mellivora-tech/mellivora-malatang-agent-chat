		...
		record(event) {
			switch (event.type) {
				...
				case 'tool_progress':
					...
					break;
				case 'subagent_start':
					agentLog.emit({ ts: now(), ...base, type: 'subagent_start', agentId: event.agentId, model: event.model, detail: { task: event.task } });
					break;
				case 'subagent_tool':
				...
				case 'stream_retry':
					...
					break;
				case 'hooks_loaded':
					agentLog.emit({ ts: now(), ...base, type: 'hooks_loaded', loaded: event.loaded, dropped: event.dropped, corrupt: event.corrupt });
					break;
				case 'hook':
					agentLog.emit({
						ts: now(),
