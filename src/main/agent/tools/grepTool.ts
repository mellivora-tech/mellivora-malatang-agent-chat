	...
	return defineTool({
		...
		inputSchema: {
			...
			properties: {
				...
				context: { type: 'integer', minimum: 0, maximum: MAX_CONTEXT, description: `Lines of surrounding context to include on each side of a match (0–${MAX_CONTEXT}, default 0). Prefer this over a follow-up read_file when you just need to see a match in situ.` },
			},
			required: ['pattern'],
