...
const server = createServer((request, response) => {
	...
	if (request.method === 'POST' && request.url === '/v1/chat/completions') {
		...
		request.on('end', () => {
			...
			if (hasTools && tooledCalls < 2) {
				tooledCalls += 1;
				const args = tooledCalls === 1 ? UI_ARGS : UI_ARGS.replace('订单迁移映射', '订单迁移映射 v2');
				response.write(`data: ${JSON.stringify({ choices: [{ delta: { tool_calls: [{ index: 0, id: 'call-' + tooledCalls, type: 'function', function: { name: 'render_ui', arguments: args } }] } }] })}\n\n`);
